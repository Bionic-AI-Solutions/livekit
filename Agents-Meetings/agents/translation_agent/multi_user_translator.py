"""
Multi-user translator implementation based on LiveKit example
Enhanced with Langfuse tracking
"""
import asyncio
import logging
...
class Translator:
    ...
    def __init__(
        ...
        *,
        ...
        self._tracer = trace.get_tracer("translation_agent")
    ...
    async def _run(self):
        """Translate sentences to target language with Langfuse tracking."""
        async for sentence in self._input_ch:
            if not sentence:
                continue

            with self._tracer.start_as_current_span("translate") as span:
                span.set_attribute("translation.source_text", sentence)
                span.set_attribute("translation.target_language", self._target_language.code)
                ...
                try:
                    ...
                    span.set_attribute("translation.translated_text", response)
    ...
    async def _synthesize_tts(self):
        async for sentence in self._llm_stream:
            with self._tracer.start_as_current_span("tts_synthesis") as span:
                span.set_attribute("tts.text", sentence)
                span.set_attribute("tts.language", self._target_language.code)
    ...
    async def _publish_and_playout(self):
        """Publish the translated audio frames to the room."""
        ...
        async for ev in self._stt_stream:
            if ev.type == stt.SpeechEventType.FINAL_TRANSCRIPT:
                with self._tracer.start_as_current_span("stt_transcription") as span:
                    span.set_attribute("stt.text", ev.alternatives[0].text)
                    span.set_attribute("stt.language", self.language)
...
class RoomTranslator:
    """Translates audio tracks in a room to multiple languages."""
